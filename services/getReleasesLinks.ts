...
const getLinks = async () => {
  ...
  for (let release of response) {
    if (releaseLinks.dmg && releaseLinks.deb && releaseLinks.exe) break;
    const { assets } = release;
    if (assets.length < 0) continue;

    assets.forEach((asset: any) => {
      const { browser_download_url } = asset;
      const match = (browser_download_url as string).match(/\.(dmg|deb|exe)$/i);
      if (!match) return;
      releaseLinks[match[0].substring(1) as keyof typeof releaseLinks] ??= [
